...
import org.springframework.security.access.AccessDeniedException; // ← NIEUW
import org.springframework.security.core.Authentication; // ← NIEUW
import org.springframework.stereotype.Service;
import org.springframework.transaction.annotation.Transactional;

import java.time.LocalDate;
import java.util.List;
import java.util.Objects; // ← NIEUW
...
public class LoanService {
    ...
    // ← GEWIJZIGD: controleert of klant alleen eigen leningen opvraagt
    public List<LoanResponse> getLoansByCustomerId(
            Long customerId,
            Authentication authentication) {

        // Controleer of de ingelogde gebruiker een beheerder is
        boolean isBeheerder = authentication.getAuthorities()
                .stream()
                .anyMatch(authority ->
                        authority.getAuthority().equals("ROLE_BEHEERDER"));

        // Klant ophalen — gooit 404 als niet gevonden
        Customer customer = customerRepository.findById(customerId)
                .orElseThrow(() ->
                        new RecordNotFoundException(
                                "Klant met id " + customerId + " niet gevonden"));

        if (!isBeheerder) {
            // Klant mag alleen zijn eigen leningen opvragen
            // authentication.getName() geeft het sub-claim uit het JWT-token (= keycloakId)
            String keycloakId = authentication.getName();

            if (!Objects.equals(customer.getKeycloakId(), keycloakId)) {
                throw new AccessDeniedException(
                        "Geen toegang tot leningen van andere klanten");
            }
        }
    ...
    public LoanResponse getLoanById(Long id) {
        Loan loan = loanRepository.findById(id)
                .orElseThrow(() -> new RecordNotFoundException(
                        "Lening met id " + id + " niet gevonden"));
    ...
    public LoanResponse createLoan(LoanRequest request) {
        ...
        BookCopy bookCopy = bookCopyRepository.findById(request.bookCopyId())
                .orElseThrow(() -> new RecordNotFoundException(
                        "Exemplaar met id " + request.bookCopyId() + " niet gevonden"));

        // Businessregel: exemplaar moet de status AVAILABLE hebben
        if (bookCopy.getStatus() != BookCopyStatus.AVAILABLE) {
            throw new BadRequestException(
                    "Exemplaar met id " + request.bookCopyId() + " is niet beschikbaar");
        }

        // Klant ophalen
        Customer customer = customerRepository.findById(request.customerId())
                .orElseThrow(() -> new RecordNotFoundException(
                        "Klant met id " + request.customerId() + " niet gevonden"));
    ...
    public LoanResponse returnLoan(Long id) {
        ...
        if (loan.getReturnDate() != null) {
            throw new BadRequestException(
                    "Lening met id " + id + " is al ingeleverd");
        }
